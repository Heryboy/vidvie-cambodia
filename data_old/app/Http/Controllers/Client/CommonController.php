...
class CommonController extends Controller
{
    ...
    public function career()
    {
        ...
        $career = DB::table('career as c')
                          ...
                          ->where('c.is_active',1)
                          ->where('cd.language_id',$language_id)
                          ->select('cd.*','c.image as image','c.id as cid','c.report_to as report_to','c.job_code as job_code','c.is_active as is_active')
                          ->get();
                          //dd($career);
    ...
    public function manage_booking(){
       $language_id = Session::get('applangId');
        ...
        $id = 4;
        $article_title = $this-> getTitleArticle($id, $language_id);
        return view('Client.manage_booking')->with('languages',$languages)
                                          ->with('article_title',$article_title)
                                          ->with('id',$id)
                                          ->with('widget',$widget)
                                          ->with('topMenu',$topMenu)
    ...
    public function current_members(){
        ...
        $id = 10;
        $article_title = $this-> getTitleArticle($id, $language_id);
        return view('Client.current_members')->with('languages',$languages)
                                      ->with('article_title',$article_title)
                                      ->with('widget',$widget)
                                      ->with('topMenu',$topMenu)
                                      ->with('mainMenu',$mainMenu)
